...
int main(int argc, char* argv[])
{
  ...
  // Particle loop
  auto ploop_iterator_k0a0 = particles.getDomainIterator();
  while (ploop_iterator_k0a0.isNext())
  {
    auto p_force = ploop_iterator_k0a0.get();
    ...
    while (nlist.isNext())
    {
      ...
      Point<3, double> neighborPos = particles.getPos(q_force);
      Point<3, double> diff = (selfPosition - neighborPos);
  ...
  for (size_t time_step = 0; time_step < 10000; time_step++)
  {
    ...
    // Particle loop
    auto ploop_iterator_a0l0a0 = particles.getDomainIterator();
    while (ploop_iterator_a0l0a0.isNext())
    {
      auto p_velocity = ploop_iterator_a0l0a0.get();
      ...
      particles.getPos(p_velocity)[0] += (dt * particles.template getProp<velocity>(p_velocity)[0]);
      particles.getPos(p_velocity)[1] += (dt * particles.template getProp<velocity>(p_velocity)[1]);
      particles.getPos(p_velocity)[2] += (dt * particles.template getProp<velocity>(p_velocity)[2]);
      particles.template getProp<force>(p_velocity)[0] = 0.0;
      particles.template getProp<force>(p_velocity)[1] = 0.0;
      particles.template getProp<force>(p_velocity)[2] = 0.0;
      ++ploop_iterator_a0l0a0;
    }
    particles.map();
    ...
    // Particle loop
    auto ploop_iterator_e0l0a0 = particles.getDomainIterator();
    while (ploop_iterator_e0l0a0.isNext())
    {
      auto p_force = ploop_iterator_e0l0a0.get();
      Point<3, double> selfPosition = particles.getPos(p_force);
      auto nlist = particles_cellList_0.template getNNIterator<NO_CHECK>(particles_cellList_0.getCell(selfPosition));
      ...
      particles.template getProp<velocity>(p_force)[0] += ((0.5 * dt) * particles.template getProp<force>(p_force)[0]);
      particles.template getProp<velocity>(p_force)[1] += ((0.5 * dt) * particles.template getProp<force>(p_force)[1]);
      particles.template getProp<velocity>(p_force)[2] += ((0.5 * dt) * particles.template getProp<force>(p_force)[2]);
      ++ploop_iterator_e0l0a0;
    }
    if (i % 100 == 0)
    {
      E = 0.0;
      ...
      // Particle loop
      auto ploop_iterator_b0f0l0a0 = particles.getDomainIterator();
      while (ploop_iterator_b0f0l0a0.isNext())
      {
        auto p_energy = ploop_iterator_b0f0l0a0.get();
        ...
        E += (((particles.template getProp<velocity>(p_energy)[0] * particles.template getProp<velocity>(p_energy)[0]) + ((particles.template getProp<velocity>(p_energy)[1] * particles.template getProp<velocity>(p_energy)[1]) + (particles.template getProp<velocity>(p_energy)[2] * particles.template getProp<velocity>(p_energy)[2]))) / 2);
        ++ploop_iterator_b0f0l0a0;
      }
      auto &vcl_c0f0l0a0 = create_vcluster();
      vcl_c0f0l0a0.sum(E);
      vcl_c0f0l0a0.execute();
      if (vcl_c0f0l0a0.getProcessUnitID() == 0)
      {
