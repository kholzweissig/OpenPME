...
int main(int argc, char* argv[])
{
  ...
  // Particle loop
  auto ploop_iterator_m0a0 = particles.getDomainIterator();
  while (ploop_iterator_m0a0.isNext())
  {
    auto p_force = ploop_iterator_m0a0.get();
  ...
  for (size_t time_step = 0; time_step < 10000; time_step++)
  {
    ...
    // Particle loop
    auto ploop_iterator_a0n0a0 = particles.getDomainIterator();
    while (ploop_iterator_a0n0a0.isNext())
    {
      auto p_velocity = ploop_iterator_a0n0a0.get();
    ...
    // Particle loop
    auto ploop_iterator_e0n0a0 = particles.getDomainIterator();
    while (ploop_iterator_e0n0a0.isNext())
    {
      auto p_force = ploop_iterator_e0n0a0.get();
      Point<3, double> selfPosition = particles.getPos(p_force);
      auto nlist = particles_cellList_0.template getNNIterator<NO_CHECK>(particles_cellList_0.getCell(selfPosition));
    ...
    if (i % 100 == 0)
    {
      ...
      // Particle loop
      auto ploop_iterator_b0f0n0a0 = particles.getDomainIterator();
      while (ploop_iterator_b0f0n0a0.isNext())
      {
        auto p_energy = ploop_iterator_b0f0n0a0.get();
        ...
        ++ploop_iterator_b0f0n0a0;
      }
      auto &vcl_c0f0n0a0 = create_vcluster();
      vcl_c0f0n0a0.sum(E);
      vcl_c0f0n0a0.execute();
      if (vcl_c0f0n0a0.getProcessUnitID() == 0)
      {
